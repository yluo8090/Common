...
- (instancetype) initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.frame = frame;
        self.alpha = 0.0f;
        self.backgroundColor = [UIColor clearColor];//[[UIColor blackColor] colorWithAlphaComponent:0.3f];
    }
    return self;
}
...
- (void) _setupBackgourndview:(CGRect)whichFrame {
    ...
    self.backgroundView.backgroundColor = [UIColor hexString:@"343434"];//[UIColor whiteColor];
...
- (void) _setupOptionContent:(UIButton *)optionButton {
    if(self.option_optionImages&&self.option_optionImages.count>0) {
        ...
        contentLabel.textColor = [UIColor hexString:@"ffffff"];//kColor(128, 128, 128);//[UIColor darkGrayColor];
        contentLabel.font = [UIFont fontWithName:PingFangMedium size:14];
        ...
        contentLabel.text = self.option_optionContents[optionButton.tag];
        contentLabel.textColor = [UIColor hexString:@"ffffff"];
        contentLabel.font = [UIFont fontWithName:PingFangMedium size:14];
    }
    
    if(optionButton.tag != 0) {
        UIView *lineView = [UIView new];
        lineView.backgroundColor = [UIColor hexString:@"666666"];
...
- (void) _tochangeBackgroudViewFrame:(CGRect)whichFrame {//这个是按钮的坐标
    ...
    CGFloat background_y = whichFrame.origin.y+which_h + 15;//140 origin.y 100  往下移动10个点
